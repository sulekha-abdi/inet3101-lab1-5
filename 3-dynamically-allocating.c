...
int main(int argc, char *argv[]) {
    ...
    int num = 0;
    while (1) {//while loop to keep prompting user for more numbers
        if (num >= array_size) {
            array_size += num_elements;
            int *temp = realloc(array, array_size * sizeof(int));
            if (!temp) {//Check if the resizing worked
                printf("error\n");//if the reallocation fails, error message printed out
                free(array);
                return 1;
            }
            array = temp;
